...
LastEditTime: 2025-09-07 00:44:11
"""
...
class ScoreAndFilterData:
    ...
    def _thread_to_silicon_get_score(self, result):
        ...
        resp_entity = {"id": id}
        if resp_array:
            resp_entity.update(resp for resp in resp_array)
        return resp_entity
    ...
    def _get_and_check_digit_return(self, gather_text, key, params, resp_array):
        ...
        while digit_batch_count < 3:
            try:
                ...
                logger.info(f"LLM({key}) got score: {score} , use time: {math.ceil(time.time() - start_time)}s.")
